...
async def move_franka_to_cube(target_color):
    ...
    json_path = os.path.join(
        CUBE_POS_DATA, f"detection_cube_{target_color.lower()}.json"
    )
    with open(json_path, "r") as f:
        data = json.load(f)
        cube_position = np.array(data["world_pos"])  # (x,y,z)
        print(
            f"##### VLM TARGETED {target_color.upper()} CUBE AT POSITION: {cube_position}"
        )

        # pre-defined height to stop the franka arm movement
        grasp_height = 0.095  # variable depending on the needs
        ...
        joint_positions = (
            franka_articulations.get_joint_positions()
        )  # important as they will be saved later on

        # save the json with the saved data
        out_path = os.path.join(
            END_POS_DIR, f"rl_ready_{target_color}_cube_franka.json"
        )
        rl_data_save = {
        ...
        print(
            f"##### DATA FOR CUBE {target_color.upper()} SAVED FOR RL IN PATH: {out_path} #####"
        )
